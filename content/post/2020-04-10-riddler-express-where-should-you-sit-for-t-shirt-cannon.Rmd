...
inverse without piecewise limiting the domain. We'll consider $\Theta$ on two 
symmetric domains -- $[0, \frac{\pi}{4}]$ and $[\frac{\pi}{4}, \frac{\pi}{2}]$ -- 
where we can consider an inverse function $h^{-1}$.

$$
\begin{aligned}
\Theta & = h^{-1}(X)  \\
h^{-1}(X) & = \begin{cases}
  \frac{1}{2}\arcsin(\frac{g}{v^2}X), && 0 \le x \le \frac{\pi}{4} \\
  \frac{1}{2}\arcsin(\frac{g}{v^2}X), && \frac{\pi}{4} \le x \le \frac{\pi}{2} \\
  \end{cases}
\end{aligned}
$$


And we'll find that our formula for the derivative has an undefined cusp.
...
Because of the symmetry in the two domains of $\Theta$ for the inverse 
function $h^{-1}$, we can simply double the probability density function
for $f_X(x)$. 

$$
\begin{aligned}
f_X(x) & = f_{\Theta}(h^{-1}(x)) \cdot \left| \frac{d}{dx}h^{-1}(x) \right| \\
& = 2 \cdot \frac{2}{\pi} \cdot \left| \frac{d}{dx}h^{-1}(x) \right| \\
& = \frac{4}{\pi} \cdot \left( \frac{g}{2v^2} \sqrt{\frac{1}{1 - (\frac{gx}{v^2})^2}} \right) \\
\end{aligned}
$$
