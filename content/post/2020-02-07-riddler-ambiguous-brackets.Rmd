...
## Results
There are `r {num_pathways}` pathways, and a prettier way to get at that 
than this code-heavy approach. But different formulations can -- and 
do -- lead to the same answer. So we have to evaluate all the paths. :(

In the end, there are only `r {num_unq_answers}` unique answers. 
(The duplicate answers are: `r {duplicate_answers}`.)
