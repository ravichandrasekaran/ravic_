...
The word *sans-serif* is a hint, telling us that we should look carefully at the letterforms. An initial cut would be to map letters based on the **number of distinct strokes** -- where one has to pick up the pen to write the letter as shown without traversing a section twice. (The letters  `J`, `A`, and `H` have one, two, and three strokes, respectively.)  Then, we can load [Peter Norvig's word list][norvig-word-list], map the letters to strokes, and find the candidate answers.

It turns out, though, that won't be enough to get a unique answer. With this mapping, we get three possible candidates: `EUREKA`, `TWEAKY`, and `APATHY`. The next cut would add a feature for the **number of enclosed regions** formed by the letter. (`J`, `A`, and `B` have zero, one, and two enclosed regions, respectively.) That gets us to a unique answer, **`EUREKA`**. 
...
Are all words unique under this scheme? Which words tend to get projected to the same encodings? The most ambiguous encoding captures the words below, including "minimum", "missing", "winning". Maybe unsurprisingly, they're composed of single-stroke, non-enclosing letterforms. 
...
A side benefit of this scheme is that it deals well with certain kinds of British vs. American English spelling, where the `S` and `Z` are equivalent.
