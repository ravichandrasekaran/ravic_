...
Then, we can load [Peter Norvig's word list][norvig-word-list], map the letters to strokes, and find the candidate answers.

It turns out, though, that won't be enough to get a unique answer. (With this mapping, we get three possible candidates: `EUREKA`, `TWEAKY`, and `APATHY`). 

The next cut would add a feature for the **number of enclosed regions** formed by the letter. That gets us to our unique answer, **`EUREKA`**. (A bit of [HARKing][harking] perhaps, but it does make for a nice, unique solution.)
